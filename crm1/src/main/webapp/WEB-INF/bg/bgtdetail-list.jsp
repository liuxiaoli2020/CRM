...
<form action="bgTaskDetailctrll/selecttdetail.do" method="post" name="myform">
	<div class="text-c"> 日期范围：
		<input type="text" value="<fmt:formatDate value="${tdetail.datemin }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" name="datemin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" value="<fmt:formatDate value="${tdetail.datemax }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d' })" id="datemax" name="datemax" class="input-text Wdate" style="width:120px;">
		<input type="text" value="${tdetail.state }" class="input-text" style="width:250px" placeholder="输入任务详情的状态" id="state" name="state">
	...
	<a href="javascript:;" onclick="member_add('添加任务详情','bgTaskDetailctrll/goaddtdetail.do','','600')" class="btn btn-primary radius">
	<i class="Hui-iconfont">&#xe600;</i> 添加任务详情</a></span> 
	<span class="r">共有数据：<strong>${p.total }</strong> 条</span> </div>
	<div class="mt-20">
	<form action="bgCheckTaskctrl/deletetdetailall.do" method="post" id="myform1">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<c:forEach items="${p.list }" var="tdetail">
				<tr class="text-c">
					<td><input type="checkbox" value="${tdetail.id }" name="ids"></td>
					<td>${tdetail.id }</td>
					<td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','360','400')">${tdetail.taskId }</u></td>
					<td>${tdetail.acceptUserId }</td>
					<td>${tdetail.isFinish }</td>
					<td>${tdetail.state }</td>
					<td>${tdetail.companyId }</td>
					<td><fmt:formatDate value="${tdetail.lastModifyDate }" pattern="yyyy-MM-dd HH:mm:ss"/></td>

					<td><a title="编辑" href="javascript:;" onclick="member_edit('编辑任务详情','bgTaskDetailctrll/goaddtdetail.do','${tdetail.id }','','600')" class="ml-5" style="text-decoration:none">
					<i class="Hui-iconfont">&#xe6df;</i></a> 
					<a title="删除" href="javascript:;" onclick="member_del(this,'${tdetail.id }')" class="ml-5" style="text-decoration:none">
...
/*考核任务-查看*/
function 任务详情(title,url,id,w,h){
	layer_show(title,url,w,h);
}
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: 'bgTaskDetailctrll/deletetdetail.do',
			data: "id="+id,
			dataType: 'json',
