...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 办公管理 <span class="c-gray en">&gt;</span> 考核任务管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
<form action="bgCheckTaskctrl/selecttask.do" method="post" name="myform">
	<div class="text-c"> 日期范围：
		<input type="text" value="<fmt:formatDate value="${task.datemin }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" name="datemin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" value="<fmt:formatDate value="${task.datemax }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d' })" id="datemax" name="datemax" class="input-text Wdate" style="width:120px;">
		<input type="text" value="${task.taskTitle }" class="input-text" style="width:250px" placeholder="输入考核任务" id="taskTitle" name="taskTitle">
	...
	<a href="javascript:;" onclick="member_add('添加考核任务','bgCheckTaskctrl/goaddtask.do','','600')" class="btn btn-primary radius">
	<i class="Hui-iconfont">&#xe600;</i> 添加考核任务</a></span> 
	<span class="r">共有数据：<strong>${p.total }</strong> 条</span> </div>
	<div class="mt-20">
	<form action="bgCheckTaskctrl/deletetaskall.do" method="post" id="myform1">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="20">ID</th>
				<th width="80">任务标题</th>
				<th width="60">任务具体内容</th>
				<th width="40">任务发布人</th>
				<th width="100">考核指标</th>
				<th width="50">任务开始时间</th>
				<th width="50">任务结束时间</th>
		...
		<tbody>
			<c:forEach items="${p.list }" var="task">
				<tr class="text-c">
					...
					<td>${task.taskPublisher }</td>
					<td class="text-l">${task.taskTarget }</td>
					<td>${task.taskStatr }</td>
					<td>${task.taskFinish }</td>
					<td>${task.companyId }</td>
					<td><fmt:formatDate value="${task.lastModifyDate }" pattern="yyyy-MM-dd HH:mm:ss"/></td>

					<td><a title="编辑" href="javascript:;" onclick="member_edit('编辑考核任务','bgCheckTaskctrl/goaddtask.do','${task.taskId }','','600')" class="ml-5" style="text-decoration:none">
					<i class="Hui-iconfont">&#xe6df;</i></a> 
					<a title="删除" href="javascript:;" onclick="member_del(this,'${task.taskId }')" class="ml-5" style="text-decoration:none">
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: 'bgCheckTaskctrl/deletetask.do',
			data: "taskId="+id,
			dataType: 'json',
