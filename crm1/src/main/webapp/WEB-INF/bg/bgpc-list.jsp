...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 办公管理 <span class="c-gray en">&gt;</span> 个人日程管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
<form action="bgPrivateCalenderctrl/selectpc.do" method="post" name="myform">
	<div class="text-c"> 日期范围：
		<input type="text" value="<fmt:formatDate value="${pc.datemin }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" name="datemin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" value="<fmt:formatDate value="${pc.datemax }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d' })" id="datemax" name="datemax" class="input-text Wdate" style="width:120px;">
		<input type="text" value="${pc.calendarTitle }" class="input-text" style="width:250px" placeholder="输入个人日程安排主题" id="calendarTitle" name="calendarTitle">
	...
	<a href="javascript:;" onclick="member_add('添加个人日程','bgPrivateCalenderctrl/goaddpc.do','','600')" class="btn btn-primary radius">
	<i class="Hui-iconfont">&#xe600;</i> 添加个人日程</a></span> 
	<span class="r">共有数据：<strong>${p.total }</strong> 条</span> </div>
	<div class="mt-20">
	<form action="bgPrivateCalenderctrl/deletepcall.do" method="post" id="myform1">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="20">开始日期</th>
				<th width="20">开始时间</th>
				<th width="20">填写人员编号</th>
				<th width="150">安排描述</th>
		...
		<tbody>
			<c:forEach items="${p.list }" var="pc">
				<tr class="text-c">
					...
					<a title="删除" href="javascript:;" onclick="member_del(this,'${pc.calendarId }')" class="ml-5" style="text-decoration:none">
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: 'bgPrivateCalenderctrl/deletepc.do',
			data: "calendarId="+id,
			dataType: 'json',
