...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 办公管理 <span class="c-gray en">&gt;</span> 短消息管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
<form action="bgMsgctrl/selectmsg.do" method="post" name="myform">
	<div class="text-c"> 日期范围：
		<input type="text" value="<fmt:formatDate value="${msg.datemin }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" name="datemin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" value="<fmt:formatDate value="${msg.datemax }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d' })" id="datemax" name="datemax" class="input-text Wdate" style="width:120px;">
		<input type="text" value="${msg.msgTitle }" class="input-text" style="width:250px" placeholder="输入短消息标题" id="msgTitle" name="msgTitle">
	...
	<a href="javascript:;" onclick="member_add('添加短消息','bgMsgctrl/goaddmsg.do','','500')" class="btn btn-primary radius">
	<i class="Hui-iconfont">&#xe600;</i> 添加短消息</a></span> 
	<span class="r">共有数据：<strong>${p.total }</strong> 条</span> </div>
	<div class="mt-20">
	<form action="bgMsgctrl/deletemsgall.do" method="post" id="myform1">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<c:forEach items="${p.list }" var="msg">
				<tr class="text-c">
					<td><input type="checkbox" value="${msg.msgId }" name="ids"></td>
					<td>${msg.msgId }</td>
					<td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','360','400')">${msg.msgTitle }</u></td>
					<td>${msg.msgDetail }</td>
					<td>${msg.msgSender }</td>
					<td>${msg.companyId }</td>
