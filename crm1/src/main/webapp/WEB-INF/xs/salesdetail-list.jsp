...
<div class="page-container">
   <form action="salesdetailctrl/selectsalesdetail.do" method="post" id="myform">
	<div class="text-c"> 日期范围：
		<input type="text" value="<fmt:formatDate value="${salesdetail.datemin }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" name="datemin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" value="<fmt:formatDate value="${salesdetail.datemax }" pattern="yyyy-MM-dd"/>" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d' })" id="datemax" name="datemax" class="input-text Wdate" style="width:120px;">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
	 <a href="javascript:;" onclick="member_add('添加客户','salesdetailctrl/goaddsalesdetail.do','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加客户</a></span>
	  <span class="r">共有数据：<strong>${p.total}</strong> 条</span> </div>
	<div class="mt-20">
	<form action="salesdetailctrl//deletecustomercontactall.do" method="post" id="myform1">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
		 <c:forEach items="${p.list }" var="salesdetail">
			<tr class="text-c">
				<td><input type="checkbox" value="${salesdetail.salesDetailId }" name="ids"></td>
				<td>${salesdetail.salesDetailId }</td>>
				<td>${salesdetail.salesId }</td>>
				<td><u style="cursor:pointer" class="text-primary" >${salesdetail.goodsId }</u></td>
				<td>${salesdetail.goodsNumber }</td>
				<td>${salesdetail.goodsPrice }</td>
				<td>${salesdetail.noteInformation }</td>
				<td>${salesdetail.companyId }</td>
				
				<td><fmt:formatDate value="${salesdetail.lastModifyDate }" pattern="yyyy-MM-dd HH:mm:ss"/></td>>
				
				<td><a title="编辑" href="javascript:;" onclick="member_edit('编辑用户','salesdetailctrl//goaddsalesdetail.do','${salesdetail.salesDetailId }','','350')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>  
				<a title="删除" href="javascript:;" onclick="member_del(this,'${customercontact.salesDetailId }')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>>
			</tr>
			</c:forEach>
		</tbody>
	</table>
