...
<body>
	...
	用户中心 <span class="c-gray en">&gt;</span> 用户账号管理 <a
		class="btn btn-success radius r"
		style="line-height:1.6em;margin-top:3px"
		href="javascript:location.replace(location.href);" title="刷新"><i
		class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="pd-20">
		<form action="xtuseraccountctrl/seletextuseraccountctrl.do" method="post" id="myfrom">
			<div class="text-c">
				日期范围： <input type="text" value="<fmt:formatDate value="${UA.datemin }"
								pattern="yyyy-MM-dd " />"
					onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}'})"
					id="datemin" id="datemin" class="input-text Wdate" style="width:120px;">
				- <input type="text" value="<fmt:formatDate value="${UA.datemax }"
								pattern="yyyy-MM-dd " />"
					onfocus="WdatePicker({minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d'})"
					id="datemax" id="datemax" class="input-text Wdate" style="width:120px;">
				<input type="text" class="input-text" style="width:250px"
					placeholder="输入用户名称。。" id="selectname" name="selectname">
					<input type="hidden" id="pageNum" name="pageNum" value="${p.pageNum }">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="datadel()"
				...
				<a href="javascript:;" class="btn btn-primary radius" id="lay_add"><i
					class="icon-plus"></i> 添加用户</a></span> <span class="r">共有数据：<strong>88</strong>
				条
			</span>
		</div>
		<!-- 添加账户 -->
		...
		<form action="xtuseraccountctrl/deletextuseraccountall.do" method="post" id="myform1">
		<table
			...
			<tbody>
				<c:forEach items="${p.list }" var="useraccount">
					<tr class="text-c">
						<td><input type="checkbox" value="" name="one_check"></td>
			...
			<!-- <button onclick="admin(3)">ceshi</button> -->
		</table>
		</form>
		...
		当前 ${p.pagenum } / ${p.pages} }页 ，共 ${p.total } 条</div>
		<div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate" style="float: right; margin-top: 10px;margin-bottom: 10px">
			<span> 
				...
				href="javascript:void(0)" onclick="gopage(${p.navigateFirstPage  }) ">首页</a>
			</span>
			...
			href="javascript:void(0)" onclick="gopage(${p.prePage  }) ">
				上一页
			</a>
			<span>
			...
			href="javascript:void(0)" onclick="gopage(${p.nextPage  }) ">
				下一页
			</a>
			<span>
				<a class="paginate_button current" aria-controls="DataTables_Table_0" data-dt-idx="1" tabindex="0"
